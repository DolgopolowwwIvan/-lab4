...
def main():
    # Заголовок
    st.title('Оценка решений.')
    st.info("""
    Дефицит бюджета города составляет 12 %. Вам необходимо оценить решения, предлагаемые для сокращения статьи расходов.
    """)

    # Добавляем поле для имени ПЕРЕД использованием
    ...
    df = pd.DataFrame({'Варианты решения': options})
    ...
    st.success("""
    **Инструкция:** 
    Для каждой пары вариантов распределите 12 баллов между ними в соответствии с тем, 
    насколько один вариант предпочтительнее другого. 

    **Пример:**
    - Если вариант A значительно лучше варианта B → A: 9 баллов, B: 3 балла
    - Если варианты равны → A: 6 баллов, B: 6 баллов  
    - Если вариант A немного лучше варианта B → A: 7 баллов, B: 5 баллов
        ...
        for j in range(i + 1, len(options)):
            ...
            with col1:
                ...
                score_a = st.slider(
                    ...
                    min_value=0,
                    max_value=12,
                    value=6,  # Изменил начальное значение для равномерного распределения
                    key=f'{i}_{j}_a',
            ...
            if total == 12:
                ...
                st.warning("Сумма баллов должна равняться 12!")
